...
async function defaultValidateRegistrationAccessToken(ctx, next) {
...
const validateRegistrationAccessToken = [
  ...
  async function validateRegistrationAccessToken(ctx, next) {
    const { registrationAccessToken } = instance(ctx.oidc.provider).configuration('features.registration');

    switch (registrationAccessToken && typeof registrationAccessToken) {
      case 'function': {
        await registrationAccessToken(ctx, next, defaultValidateRegistrationAccessToken);
        break;
      }
      default: {
        await defaultValidateRegistrationAccessToken(ctx, next);
        break;
      }
    }
  },
...
const setInitialProperties = async (ctx, properties) => {
  const { oidc: { provider } } = ctx;

  const { initialProperties } = instance(provider).configuration('features.registration');
  switch (initialProperties && typeof initialProperties) {
    case 'object': {
      Object.assign(properties, initialProperties);
      break;
    }
    case 'function': {
      Object.assign(properties, await initialProperties(ctx, properties));
      break;
    }
    default:
  }
...
module.exports = {
  post: [
    ...
    async function validateInitialAccessToken(ctx, next) {
      ...
      switch (initialAccessToken && typeof initialAccessToken) {
        ...
        case 'function': {
          return await initialAccessToken(ctx, next);
          break;
        }
        default:
      }
    ...
    async function registrationResponse(ctx, next) {
      ...
      await setInitialProperties(ctx, properties);
  ...
  put: [
    ...
    async function clientUpdateResponse(ctx, next) {
      ...
      if (ctx.oidc.entities.RegistrationAccessToken && ctx.oidc.entities.RegistrationAccessToken.policies) {
